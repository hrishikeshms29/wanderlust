...
def all_packages():
    if isinstance(current_user, User):
        ...
        return render_template('all_packages.html', username=current_user.Username, packages=user_packages)
...
def add_hotel():
    if isinstance(current_user, ServiceProvider):
        if request.method == 'POST':
            # Process form data and add a new hotel
            # Use Hotel model to store data in the database
            # Redirect to provider home or show a success message
